...
**`src/index.ts`** — Plugin entry point implementing the `VcsPlugin` interface. Calls `restoreStateFromLocalStorage` during `initialize`, starts sync on `onVcsAppMounted`, and tears down on `destroy`.
